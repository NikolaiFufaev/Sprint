...
import static tracker.controllers.FileBackedTasksManager.fileBack;
...
abstract class TaskManagerTest<T extends TaskManager> {
    ...
    void createEpicAndSubtask() {
        ...
        subtask.setStartTime("20.03.1985 23:55");
    ...
    void updateEpicAndSubtaskDone() {
        ...
        taskManager.createTask(epic);
        Subtask subtask1 = new Subtask("Test addNewSubtask1", "Test addNewSubtask1 description", epic.getId());
        subtask1.setStartTime("20.03.1985 23:55");
        taskManager.createTask(subtask1);
        Subtask subtask2 = new Subtask("Test addNewSubtask2", "Test addNewSubtask2 description", epic.getId());
        subtask2.setStartTime("20.03.2000 23:55");
        taskManager.createTask(subtask2);
    ...
    void saveAndLoadEmpty() {
        ...
        assertTrue(fileBackedTasksManager.getTasks().isEmpty(), "История не пуста");
        fileBackedTasksManager.save();
        fileBackedTasksManager = FileBackedTasksManager.loadFromFile(fileBack());
        assertTrue(fileBackedTasksManager.getTasks().isEmpty(), "История не пуста");
    }

    @Test
    void saveAndLoadEpic() {
        FileBackedTasksManager fileBackedTasksManager = new FileBackedTasksManager(historyManager);
        Epic epic = new Epic("Test addNewEpic", "Test addNewEpic description");
        fileBackedTasksManager.createTask(epic);
        assertFalse(fileBackedTasksManager.getTasks().isEmpty(), "История пуста");
        fileBackedTasksManager.save();
        fileBackedTasksManager = FileBackedTasksManager.loadFromFile(fileBack());
        assertTrue(fileBackedTasksManager.getTasks().contains(epic), "Файл не загружен");
    }
    ...
    void saveAndLoadEmptyHistory() {
        ...
        Epic epic = new Epic("Test addNewEpic", "Test addNewEpic description");
        fileBackedTasksManager.createTask(epic);
        assertTrue(fileBackedTasksManager.getHistory().getHistory().isEmpty(), "История не пуста");
        fileBackedTasksManager = FileBackedTasksManager.loadFromFile(fileBack());
        assertTrue(fileBackedTasksManager.getTasks().contains(epic), "Файл не загружен");
        assertTrue(fileBackedTasksManager.getHistory().getHistory().isEmpty(), "История не пуста");
    }
